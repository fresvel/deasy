...
-- ── GEOGRAFIA ────────────────────────────────────────────────────────────────────────────────────
-- Catalogo encadenado pais -> provincia -> ciudad, mas las operadoras moviles. Existe porque
...
-- La operadora es DATO DECLARADO POR LA PERSONA, NUNCA AUTORITATIVO. Con portabilidad numerica la
-- operadora de un numero cambia sin que nadie avise, asi que lo guardado se pudre solo. Si alguna
-- vez hace falta de verdad, se consulta en el momento de usarlo.
CREATE TABLE IF NOT EXISTS operadoras (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_operadoras_pais FOREIGN KEY (pais_id) REFERENCES paises(id)
);
CREATE OR REPLACE TRIGGER trg_operadoras_set_updated_at BEFORE UPDATE ON operadoras FOR EACH ROW EXECUTE FUNCTION set_updated_at();
