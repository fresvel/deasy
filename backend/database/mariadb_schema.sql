...
-- ============================================================
-- TTHH REFACTOR: nuevas columnas en persons
-- Todas usan ADD COLUMN IF NOT EXISTS → idempotente
-- ============================================================

-- Identidad personal
ALTER TABLE persons
  ADD COLUMN IF NOT EXISTS fecha_nacimiento    DATE          NULL AFTER last_name,
  ADD COLUMN IF NOT EXISTS genero              ENUM('M','F','LGBTI') NULL AFTER fecha_nacimiento,
  ADD COLUMN IF NOT EXISTS etnia               VARCHAR(60)   NULL AFTER genero,
  ADD COLUMN IF NOT EXISTS nacionalidad        VARCHAR(80)   NULL AFTER etnia,
  ADD COLUMN IF NOT EXISTS estado_civil        ENUM('Soltero','Casado','Union libre','Union de hecho','Divorciado','Viudo') NULL AFTER nacionalidad,
  ADD COLUMN IF NOT EXISTS tipo_sangre         VARCHAR(10)   NULL AFTER estado_civil,
  ADD COLUMN IF NOT EXISTS pasaporte           VARCHAR(30)   NULL AFTER tipo_sangre,
  ADD COLUMN IF NOT EXISTS tipo_visa           VARCHAR(40)   NULL AFTER pasaporte;

-- Contacto adicional
ALTER TABLE persons
  ADD COLUMN IF NOT EXISTS celular             VARCHAR(30)   NULL AFTER whatsapp;

-- Dirección domiciliaria completa
ALTER TABLE persons
  ADD COLUMN IF NOT EXISTS numero_casa         VARCHAR(20)   NULL AFTER calle_secundaria,
  ADD COLUMN IF NOT EXISTS referencia_domicilio VARCHAR(255) NULL AFTER numero_casa,
  ADD COLUMN IF NOT EXISTS sector_barrio       VARCHAR(120)  NULL AFTER referencia_domicilio,
  ADD COLUMN IF NOT EXISTS provincia_nacimiento VARCHAR(120) NULL AFTER pais,
  ADD COLUMN IF NOT EXISTS canton              VARCHAR(120)  NULL AFTER provincia_nacimiento,
  ADD COLUMN IF NOT EXISTS parroquia           VARCHAR(120)  NULL AFTER canton;

-- ============================================================
-- TTHH: tabla salud / discapacidad
-- ============================================================
CREATE TABLE IF NOT EXISTS person_health (
  id            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  person_id     INT NOT NULL,
  tiene_discapacidad        TINYINT(1)    NOT NULL DEFAULT 0,
  tipo_discapacidad         ENUM('Auditiva','Fisica','Intelectual','Lenguaje','Visual') NULL,
  porcentaje_discapacidad   DECIMAL(5,2)  NULL,
  enfermedad_catastrofica   TINYINT(1)    NOT NULL DEFAULT 0,
  created_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uq_person_health (person_id),
  CONSTRAINT fk_person_health_person
    FOREIGN KEY (person_id) REFERENCES persons(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================================
-- TTHH: tabla cuenta bancaria
-- ============================================================
CREATE TABLE IF NOT EXISTS person_bank_accounts (
  ...
  created_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_bank_person (person_id),
  CONSTRAINT fk_bank_person
    FOREIGN KEY (person_id) REFERENCES persons(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ============================================================
-- TTHH: tabla contacto de emergencia
-- ============================================================
CREATE TABLE IF NOT EXISTS person_emergency_contacts (
  ...
  created_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uq_emergency_person (person_id),
