...
const onfileChange = (type) => {
  if (type == 'grades' && file_grades.value?.files?.length > 0) {
    const file = file_grades.value.files[0];
    if (!file.name.toLowerCase().endsWith('.csv')) {
      alert('Solo se permiten archivos CSV');
      file_grades.value.value = '';
      return;
    }
    csv_grades.value = file.name;
  } else if (type == 'tutorias' && file_tutorias.value?.files?.length > 0) {
    const file = file_tutorias.value.files[0];
    if (!file.name.toLowerCase().endsWith('.csv')) {
      alert('Solo se permiten archivos CSV');
      file_tutorias.value.value = '';
      return;
    }
    csv_tutorias.value = file.name;
  }
}
