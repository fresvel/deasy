      ...
      <div v-show="!h.isDeliverableCollapsed(deliverable.item)" class="mt-3 flex flex-col gap-3">
        ...
        <div class="mt-auto grid grid-cols-[minmax(0,1fr)_auto] gap-2.5 border-t border-line pt-3">
          ...
          <div class="flex h-full items-center justify-end gap-1.5">
            <AppButton v-if="!h.shouldShowStartDeliverable(deliverable.item) && h.canApproveFillRequestForPayload(deliverable.item)" variant="success-soft" size="sm" :disabled="fillWorkflowSubmitting" :aria-label="h.getFillApproveActionLabelForPayload(deliverable.item)" @click="emit('approve', deliverable.item)" icon-only><IconCircleCheck class="h-[1.15rem] w-[1.15rem]" /></AppButton>
            <AppButton v-else-if="!h.shouldShowStartDeliverable(deliverable.item) && h.getDeliverableSubject(deliverable.item).preloadFilePath" variant="info-soft" size="sm" aria-label="Descargar PDF" @click="emit('download', deliverable.item)" icon-only><IconDownload class="h-[1.15rem] w-[1.15rem]" /></AppButton>
            <AppButton v-else-if="!h.shouldShowStartDeliverable(deliverable.item) && h.shouldShowTemplateDownload(deliverable.item)" variant="info-soft" size="sm" aria-label="Descargar plantilla" @click="emit('template', deliverable.item)" icon-only><IconFileDescription class="h-[1.15rem] w-[1.15rem]" /></AppButton>
            <AppButton v-if="!h.shouldShowStartDeliverable(deliverable.item) && h.getDeliverableSubject(deliverable.item).preloadPdfPath" variant="info-soft" size="sm" aria-label="Ver PDF" @click="emit('preview', deliverable.item)" icon-only><IconEye class="h-[1.15rem] w-[1.15rem]" /></AppButton>
