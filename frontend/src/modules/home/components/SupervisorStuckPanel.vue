<template>
  ...
  <section
    v-if="isSupervisor"
    class="bg-white rounded-xl shadow-xl shadow-slate-200/40 p-5 md:p-6 border border-line flex flex-col gap-4"
  >
    <div class="flex items-start justify-between gap-3">
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1 rounded-2xl border border-line px-2.5 py-1 text-xs font-semibold text-icon transition hover:border-indigo-300 hover:bg-indigo-50 disabled:opacity-50"
        :disabled="loading"
        @click="load"
      >↻ Actualizar</button>
    </div>

    <div v-if="loading" class="rounded-xl border border-line bg-surface px-4 py-4 text-sm font-medium text-slate-500">
      Cargando…
    </div>
