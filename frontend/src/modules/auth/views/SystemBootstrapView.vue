<template>
  <AuthLayout size="2xl">
    ...
    <div v-if="mode === 'bootstrap'" class="space-y-6" autocomplete="off">
      ...
      <div v-show="step === 1" class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <input :id="fieldId('email-1')" v-model="form.email" type="email" autocomplete="off" class="deasy-control" placeholder="admin@institucion.edu.ec" />
          </div>
          <div>
            <label :for="fieldId('first-name-1')" class="deasy-form-label">Nombres</label>
            <input :id="fieldId('first-name-1')" v-model="form.first_name" type="text" autocomplete="off" class="deasy-control" placeholder="Administrador" />
          </div>
          <div>
            <label :for="fieldId('last-name-1')" class="deasy-form-label">Apellidos</label>
            <input :id="fieldId('last-name-1')" v-model="form.last_name" type="text" autocomplete="off" class="deasy-control" placeholder="Principal" />
          </div>
          <div>
            <label :for="fieldId('whatsapp')" class="deasy-form-label">WhatsApp (opcional)</label>
            <input :id="fieldId('whatsapp')" v-model="form.whatsapp" type="text" inputmode="tel" autocomplete="off" class="deasy-control" placeholder="0990000000" />
          </div>
          <div class="hidden md:block"></div>
          <div>
            <label :for="fieldId('password-1')" class="deasy-form-label">Contraseña</label>
            <input :id="fieldId('password-1')" v-model="form.password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Nueva contraseña" />
          </div>
          <div>
            <label :for="fieldId('confirm-password-1')" class="deasy-form-label">Confirmar contraseña</label>
            <input :id="fieldId('confirm-password-1')" v-model="form.confirm_password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Repite la contraseña" />
          </div>
        </div>
      </div>
        ...
        <div v-if="gestorEnabled" class="space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <input :id="fieldId('email-2')" v-model="gestorForm.email" type="email" autocomplete="off" class="deasy-control" placeholder="gestor@institucion.edu.ec" />
            </div>
            <div>
              <label :for="fieldId('first-name-2')" class="deasy-form-label">Nombres</label>
              <input :id="fieldId('first-name-2')" v-model="gestorForm.first_name" type="text" autocomplete="off" class="deasy-control" placeholder="Gestor" />
            </div>
            <div>
              <label :for="fieldId('last-name-2')" class="deasy-form-label">Apellidos</label>
              <input :id="fieldId('last-name-2')" v-model="gestorForm.last_name" type="text" autocomplete="off" class="deasy-control" placeholder="Procesos" />
            </div>
            <div>
              <label :for="fieldId('password-2')" class="deasy-form-label">Contraseña</label>
              <input :id="fieldId('password-2')" v-model="gestorForm.password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Contraseña del gestor" />
            </div>
            <div>
              <label :for="fieldId('confirm-password-2')" class="deasy-form-label">Confirmar contraseña</label>
              <input :id="fieldId('confirm-password-2')" v-model="gestorForm.confirm_password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Repite la contraseña" />
            </div>
          </div>
        </div>
        ...
        <div v-if="usuarioEnabled" class="space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <input :id="fieldId('email-3')" v-model="usuarioForm.email" type="email" autocomplete="off" class="deasy-control" placeholder="usuario@institucion.edu.ec" />
            </div>
            <div>
              <label :for="fieldId('first-name-3')" class="deasy-form-label">Nombres</label>
              <input :id="fieldId('first-name-3')" v-model="usuarioForm.first_name" type="text" autocomplete="off" class="deasy-control" placeholder="Usuario" />
            </div>
            <div>
              <label :for="fieldId('last-name-3')" class="deasy-form-label">Apellidos</label>
              <input :id="fieldId('last-name-3')" v-model="usuarioForm.last_name" type="text" autocomplete="off" class="deasy-control" placeholder="Prueba" />
            </div>
            <div>
              <label :for="fieldId('password-3')" class="deasy-form-label">Contraseña</label>
              <input :id="fieldId('password-3')" v-model="usuarioForm.password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Contraseña del usuario" />
            </div>
            <div>
              <label :for="fieldId('confirm-password-3')" class="deasy-form-label">Confirmar contraseña</label>
              <input :id="fieldId('confirm-password-3')" v-model="usuarioForm.confirm_password" type="password" autocomplete="new-password" class="deasy-control" placeholder="Repite la contraseña" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between gap-3 pt-1">
        <button v-if="step > 1" type="button" class="deasy-btn deasy-btn--neutral-outline" @click="prevStep">Atrás</button>
        <span v-else></span>
        <button v-if="step < steps.length" type="button" class="deasy-btn deasy-btn--primary-outline" :disabled="!canAdvance" @click="nextStep">
          Siguiente <IconArrowRight class="h-5 w-5" />
        </button>
        <button v-else type="button" class="deasy-btn deasy-btn--primary-outline" :disabled="isSubmitting" @click="submitBootstrap">
  ...
  npm run recover:admin -- \
    ...
    <div v-if="mode === 'normal'" class="mt-8 flex justify-center">
      <button type="button" class="deasy-btn deasy-btn--neutral-outline" @click="router.replace({ name: 'login' })">
        Ir al login
      </button>
    </div>
  </AuthLayout>
