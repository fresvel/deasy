  ...
  <div class="unit-graph-view flex flex-col gap-3">
    <div class="flex flex-wrap items-center justify-between gap-2">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <label class="flex items-center gap-1.5 text-xs font-medium text-brand-icon">
          Relación
          <select
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      <label class="flex items-center gap-1.5 text-xs font-medium text-brand-icon">
        ...
        Resaltar pendientes
        <span v-if="pendingCount" class="inline-flex items-center rounded-xl bg-amber-50 px-1.5 py-0.5 text-[11px] font-bold text-state-warning ring-1 ring-amber-200">{{ pendingCount }}</span>
      </label>
    ...
    <div v-if="legend.length > 1" class="flex flex-wrap items-center gap-3">
      <span v-for="item in legend" :key="item.code" class="inline-flex items-center gap-1.5 text-xs font-medium text-brand-icon">
    ...
    >
      <p class="m-0 mb-3 text-sm text-brand-icon">Relación <strong>{{ editingEdgeLabel }}</strong>.</p>
    ...
    >
      ...
      <div class="flex flex-col gap-3">
        ...
        <label class="block text-sm font-medium text-slate-700">
          Slug <span class="font-normal text-brand-text-muted">(opcional)</span>
    ...
    <div v-if="detailUnit" class="deasy-drawer-overlay" @click.self="closeDetail">
      <aside class="deasy-drawer">
        <header class="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4">
          <div class="min-w-0">
            <p class="m-0 text-xs font-bold uppercase tracking-wide text-brand-text-muted">Detalle de unidad</p>
            <h3 class="m-0 mt-0.5 truncate text-base font-bold text-slate-800">{{ detailUnit.name }}</h3>
          </div>
          <button type="button" class="shrink-0 text-brand-text-muted transition-colors hover:text-brand-icon" title="Cerrar" @click="closeDetail">
            <IconX class="h-5 w-5" />
          </button>
        </header>
          ...
          <ul v-else class="m-0 flex list-none flex-col gap-2 p-0">
            <li v-for="pos in detailPositions" :key="pos.id" class="rounded-xl border border-brand-border px-3 py-2.5">
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs text-brand-text-muted">#{{ pos.slot_no }}</span>
                ...
                <div v-if="editable" class="ml-auto flex items-center gap-1">
                  <button type="button" class="graph-icon-btn" :class="pos.is_unit_head ? 'text-amber-500' : 'text-brand-text-muted'" title="Marcar/quitar jefatura" @click="toggleHead(pos)">
                    <IconCrown class="h-4 w-4" />
                  </button>
                  <button type="button" class="graph-icon-btn text-brand-text-muted hover:text-indigo-600" title="Editar puesto" @click="openEditPosition(pos)">
                    <IconPencil class="h-4 w-4" />
                  </button>
                  <button type="button" class="graph-icon-btn text-brand-text-muted hover:text-rose-600" title="Eliminar puesto" @click="removePosition(pos.id)">
              ...
              <div class="mt-1 flex flex-wrap items-center gap-2 text-xs">
                <template v-if="pos.person_id">
                  <span class="inline-flex items-center rounded-xl bg-emerald-50 px-2 py-0.5 font-semibold text-state-success ring-1 ring-emerald-200">Ocupado</span>
                  <span class="truncate text-brand-icon">{{ (pos.person_name || '').trim() }} · {{ pos.cedula }}</span>
                  <template v-if="editable">
              ...
              <div v-if="editable && assignForId === pos.id" class="mt-2 rounded-2xl border border-brand-border bg-brand-surface-muted p-2">
                <input
                  ...
                  @input="searchPersons"
                />
                <div v-if="personSearching" class="mt-1 px-1 text-[11px] text-brand-text-muted">Buscando…</div>
                <ul v-else-if="personResults.length" class="m-0 mt-1 flex max-h-40 list-none flex-col gap-0.5 overflow-y-auto p-0">
                  <li v-for="per in personResults" :key="per.id">
                    <button type="button" class="flex w-full items-center justify-between rounded-xl px-2 py-1 text-left text-xs hover:bg-indigo-50" @click="pickPerson(per.id)">
                      ...
                      <span class="ml-2 shrink-0 text-brand-text-muted">{{ per.cedula }}</span>
                    </button>
                  </li>
                </ul>
                <div v-else-if="personQuery.trim().length >= 2" class="mt-1 px-1 text-[11px] text-brand-text-muted">Sin resultados.</div>
              </div>
            </li>
          </ul>
          </div>
            ...
            <ul v-else class="m-0 flex list-none flex-col gap-2 p-0">
              <li v-for="proc in detailProcesses" :key="proc.rule_id" class="rounded-xl border border-brand-border px-3 py-2.5">
                ...
                <div class="mt-1.5 flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <span v-else-if="proc.origin === 'direct'" class="ml-auto text-[11px] italic text-brand-text-muted">Versiona el proceso para cambiar el alcance</span>
                  <span v-else class="ml-auto text-[11px] italic text-brand-text-muted">Definido a nivel de proceso</span>
                </div>
              </li>
            </ul>
            <p class="m-0 mt-3 text-[11px] leading-snug text-brand-text-muted">
    ...
    >
      <div class="flex flex-col gap-3">
        ...
        <label class="block text-sm font-medium text-slate-700">
          Título <span class="font-normal text-brand-text-muted">(opcional)</span>
