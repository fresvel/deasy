  ...
  <div class="flex flex-col gap-3">
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2 text-sm text-muted">
        <AppButton variant="neutral-outline" size="sm" icon-only title="Regresar" aria-label="Regresar" @click="$emit('go-back')">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <AppButton variant="neutral-outline" size="sm" icon-only :disabled="loading" title="Refrescar" aria-label="Refrescar" @click="loadGraph">
          <IconRefresh class="h-4 w-4" :class="loading ? 'animate-spin' : ''" />
        </AppButton>
        <AppButton variant="neutral-outline" size="sm" icon-only :disabled="exporting" title="Exportar PNG" aria-label="Exportar PNG" @click="exportPng">
          <IconDownload class="h-4 w-4" />
        </AppButton>
        <AppButton v-if="editable" variant="primary-outline" size="sm" :disabled="loading" title="Crear proceso raíz" @click="openCreateProcess('root', null, '')">
    ...
    <AppModalShell
      ...
      <template #footer>
        ...
        <AppButton variant="danger-outline" @click="confirmDeleteEdge">Desvincular</AppButton>
      </template>
    </AppModalShell>
    ...
    <AppModalShell
      ...
      <template #footer>
        ...
        <AppButton variant="primary-outline" :disabled="!createForm.name.trim()" @click="confirmCreateProcess">Crear</AppButton>
      </template>
    </AppModalShell>
    ...
    <AppModalShell
      ...
      <template #footer>
        <AppButton variant="danger-outline" :disabled="savingEdit" @click="closeEditModal">Cancelar</AppButton>
        <AppButton variant="primary-outline" :disabled="savingEdit || !editForm.name.trim()" @click="saveProcessEdit">{{ savingEdit ? "Guardando…" : "Guardar" }}</AppButton>
      </template>
    </AppModalShell>
          ...
          <template v-else>
            ...
            <div v-show="detailTab === 'configuraciones'">
              <div class="mb-3 flex items-center justify-between gap-2">
                ...
                <AppButton v-if="editable" variant="primary-outline" size="sm" @click="createConfiguration">+ Nueva configuración</AppButton>
              </div>
            ...
            <div v-show="detailTab === 'subprocesos'">
              <div class="mb-3 flex items-center justify-between gap-2">
                ...
                <AppButton v-if="editable" variant="neutral-outline" size="sm" @click="addChildFromDrawer">+ Sub-proceso</AppButton>
              </div>
    ...
    <div v-if="templateDetail" class="deasy-drawer-overlay" @click.self="closeTemplateVersions">
      <aside class="deasy-drawer">
        ...
        <div v-if="editable" class="flex flex-wrap gap-2 border-b border-line px-4 py-2.5">
          <AppButton variant="neutral-outline" size="sm" @click="versionFromDrawer">+ Nueva versión</AppButton>
          <AppButton v-if="templateDetail.configStatus === 'active'" variant="primary-outline" size="sm" @click="guidedFromDrawer">Actualizar (publicar + activar)</AppButton>
        </div>
