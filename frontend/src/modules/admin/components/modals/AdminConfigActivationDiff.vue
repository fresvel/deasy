      ...
      <template v-else>
        ...
        <ul class="m-0 mb-2 flex list-none flex-col gap-1 p-0">
          <li v-for="t in diff.templates" :key="t.template_code" class="flex items-center gap-2 text-xs">
            ...
            <span v-else class="text-slate-400">sin cambios · v{{ t.to_version }}</span>
          </li>
          <li v-if="!diff.templates.length" class="text-xs text-slate-400">Sin entregables.</li>
        </ul>
