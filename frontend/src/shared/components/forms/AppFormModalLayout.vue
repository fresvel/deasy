<template>
  <div class="profile-admin-skin relative w-full px-4 py-6 sm:px-6">
    ...
    <div
      v-if="errorMessage"
      class="mx-auto w-full max-w-xl rounded-2xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700"
      role="alert"
    >
      {{ errorMessage }}
    </div>
    ...
    <form class="mx-auto flex w-full max-w-xl flex-col gap-4 text-sm" @submit.prevent="$emit('submit')">
      ...
      <div class="mt-5 flex w-full flex-col-reverse justify-end gap-3 border-t border-brand-border pt-4 sm:flex-row">
        <AppButton
          type="button"
          variant="secondary"
          data-modal-dismiss
