...
| `audit_bindparams.mjs`           | 391    | Cuadra los huecos de cada consulta con sus parámetros, en **los dos dialectos** —`?` del adaptador y `$N` nativo de `pg`—. En verde: **413 llamadas decidibles, 0 desajustes**; las 66 indecidibles (SQL compuesto con `${…}`) las cierra la sonda de `bindParams`, no este gate. |
