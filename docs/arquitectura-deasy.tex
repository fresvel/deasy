...
startServer()
 ...
 +- IIFE async: SqlAdminService().reconcileArtifactWorkflows({ onlyStale: true })
\end{diagrama}
...
cada una de las 46 tablas administrables, hay tres ficheros que colaboran:

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{\rt{backend/config/sqlTables.js}} (1.009 lineas): un \textbf{catalogo
  ...
  \item \textbf{\rt{backend/services/admin/SqlAdminService.js}} (884 lineas): lee ese catalogo
        y \textbf{construye los SELECT, INSERT, UPDATE y DELETE dinamicamente}.
  \item \textbf{\rt{backend/services/admin/crud/tableHooks.js}} (1.137 lineas): los
...
\rt{getNextStorageVersionForTemplateCode}, \rt{loadTemplateArtifactMetaDocument}).
...
\rt{backend/database/postgres\_schema.sql} (1.398 lineas).
...
Estan \textbf{deprecados en la autoria web} \rt{document\_owner}, \rt{position} y
\rt{manual\_pick}: siguen en el ENUM por legado.
\end{nota}

\begin{aviso}[\rt{document\_owner} NO esta retirado: sigue vivo]
Es el resolver \textbf{mas usado} de la base de dev. La fase P1.4 retiro el atajo del
\emph{proceso por defecto} (\rt{SystemBootstrapService.js}), pero dejo el de
\rt{BASE\_META\_YAML} --- un \rt{meta.yaml} escrito a mano como literal de codigo unas 250
lineas mas arriba, \textbf{en el mismo fichero}. Y ademas \textbf{se auto-replica}:
\rt{createTemplateArtifactVersion} copia los objetos de MinIO en binario, asi que cada version
nueva lo hereda.

\textbf{No lo des por muerto sin consultar \rt{fill\_flow\_steps}.} El plan para retirarlo es el
frente 0 de \rt{docs/planes/plan-maestro-2026-08.md}.
\end{aviso}
...
con \rt{schema.json}, \rt{meta.yaml}, \rt{data.yaml}, \rt{main.tex.j2} y \rt{make.sh}, y valida
el pipeline: render Jinja2 con \rt{StrictUndefined} $\rightarrow$ \rt{pdflatex} $\rightarrow$ PDF.
...
Falta \emph{cablear} el render Jinja2 a PDF en tiempo de ejecucion: hoy \rt{render\_seed.py} es
utileria de linea de comandos, y el backend no lo invoca.
\end{nota}

\subsection{Ciclo de vida y versionado}

\rt{template\_artifacts.lifecycle\_state} admite \rt{draft}, \rt{published} y \rt{retired}
(por defecto \rt{published}). El versionado es \textbf{por linaje}: una fila nueva con
...
La columna \rt{anchor\_refs} (JSONB) enlaza el paso con anclas visuales dentro del PDF, es
decir, donde se dibuja la firma.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Un test unitario nuevo tiene que \textbf{caer dentro de los globs de
        \rt{backend/package.json} $\rightarrow$ \rt{test:unit}} (\rt{config/}, \rt{services/**},
        \rt{utils/**}, \rt{middlewares/**}, \rt{controllers/**}, \rt{errors/**}). Si lo pones
...
\textbf{Backend}: 22 ficheros, \textbf{361 casos}, con \rt{node --test} --- el runner nativo de
...
\rt{services/documents/FillRequestWorkflowService.test.js}  & 37 \\
\rt{services/admin/crud/validation.test.js}                 & 33 \\
\rt{services/admin/templates/workflows.test.js}             & 28 \\
\rt{services/sign/PdfSigningService.test.js}                & 27 \\
\rt{services/sign/BatchSigningService.test.js}              & 21 \\
\rt{controllers/users/user\_controler.primitives.test.js}   & 21 \\
\rt{services/tasks/GeneralTaskService.test.js}              & 19 \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{Frontend}: 18 ficheros, unos \textbf{261 casos} con Vitest y \rt{@vue/test-utils}. La
...
En Deasy son \textbf{15 suites, 150 tests y 256 claves de snapshot}, sin dependencias externas:
...
\rt{\_\_snapshots\_\_/} &
  15 JSON, uno por suite, con claves ordenadas alfabeticamente e indentacion 2, para que el
...
Los prefijos \rt{zz}, \rt{zzz} y \rt{zzzz} en los nombres de fichero \textbf{fuerzan el orden
alfabetico} de ejecucion (el runner corre con \rt{--test-concurrency=1}), poniendo al final las
suites que \emph{escriben} datos: \rt{zz\_task\_generation}, \rt{zz\_template\_lifecycle},
...
\begin{enumerate}[leftmargin=1.8em,itemsep=6pt]
  ...
  \item \textbf{El \rt{README.md} raiz enlaza documentos que ya no existen}
        ...
        \rt{docs/01-arquitectura/}). Estan en \rt{docs/docs-md-antiguos/}. Lo unico que hay hoy
        en \rt{docs/02-dominio-datos/} es \rt{consolidado.dbml} (31 KB, con unas 205 relaciones).
