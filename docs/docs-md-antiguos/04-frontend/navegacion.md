...
- **`core/router/`**: Configuración de vistas (Rutas registradas: `/`, `/home`, `/perfil`, `/register`, `/firmar`, `/admin`).
- **`core/services/`**: Servicios base y clientes compartidos (aquí transita también toda la gestión del `logged` del home).
...
- **`home/`**: Módulo del espacio de trabajo del usuario que abarca el Panel Operativo, las lógicas de tareas y flujos de "ProcessDefinition".
...
- **Rutas Públicas**: `/` (vista de login) y `/register`. Si hay un token válido presente y alguien se direcciona aquí, se le re-conduce inmediatamente a `/home`.
...
### Home: Variación de Estado Interno
La ruta maestra `/home` aloja dos flujos interactivos dentro del mismo lienzo (`HomeView`):
