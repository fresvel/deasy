...
| **6** | **El versionado copia FILAS, no bytes**: `createTemplateArtifactVersion` (`templateArtifact.js:336-409`) y `forkDeliverableForConfig` (`templateLifecycle.js:531-610`). Aquí se corta la auto-replicación de `document_owner` (§0.3).<br><br>⚠️ **Ya está medido cuánto falta**: en el experimento del sub-paso 4, contar solo por artifact tumbó `guided_update_finish` con un 400 — porque el borrador que produce el update guiado tiene su `meta.yaml` con el flujo y **cero filas colgadas del artifact**. **Es la prueba de que este sub-paso hace falta**, y de que hasta él el `OR` del gate no se puede quitar | **No** | Char nuevo: versionar con flujo → la hija tiene los mismos pasos con ids distintos. **Debe cubrir entrega Y firma** |
