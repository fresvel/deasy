...
| **3** | **Escritura doble.** `saveTemplateArtifactDraft` escribe el flujo **en la base** (colgando de `template_artifact_id`) dentro de una transacción, **y sigue emitiendo el `meta.yaml` igual que hoy**. Las dos copias salen del **mismo objeto en memoria**: no pueden divergir. **El escalón 2 sigue ganando**, así que el runtime no nota nada.<br><br>⚠️ **Las columnas del 1-bis no bastan solas:** `normalizeFillSteps` (`workflows.js:234-248`) **no propaga `code` ni `name`** a su objeto de salida, y el `INSERT` de `workflowSync.js:246` **tampoco los lista**. Hay que tocar los dos sitios o los nombres se siguen perdiendo con la columna puesta.<br><br>Aquí es donde `code` y `name` **entran en el golden `flow_steps_db.json`**, en el mismo commit que empieza a escribirlos | Sí | char sin diffs en los **pasos**; el golden del sub-paso 0 gana cabeceras nuevas (las de plantilla), y **eso es la prueba** |
