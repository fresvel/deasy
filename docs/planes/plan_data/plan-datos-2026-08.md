...
| `TD7-c4` | D7 | El mismo hecho, guardado dos veces en `chat_notifications` | ⬜ | Su único escritor (`ChatNotificationService.js:33`) pone `entity_type='conversation'` + `entity_id=String(conversation.id)` **y además** `conversation_id`. Columna tipada y par polimórfico cargando el mismo dato. Es deuda del modelo de notificaciones, no de claves ajenas | — |
