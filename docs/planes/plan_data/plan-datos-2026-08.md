...
| `TD7-c3` | D7 | **Decisión del dueño**: ¿las «FKs lógicas» del chat y el dossier al núcleo se quedan así? | ✅ | **No.** «Que se lo impida» (2026-08-24). La premisa de «no acoplar» era falsa: `ChatAuthorizationService` ya resuelve los permisos con `INNER JOIN` contra `units`/`processes`/`process_definition_versions`. Las **11** columnas —once, no diez: el censo no vio `created_by` por no acabar en `_id`— llevan su restricción, con la política por defecto salvo `dossiers.person_id` (CASCADE). **Probado por mutación con control**: una persona sin rastro se borra; la misma con un mensaje de chat, la base lo impide por nombre; y un expediente sí se va con su persona. El chat pasa de **0 a 10** referencias salientes en el modelo | 2026-08-24 |
| `TD7-r` | D7 | **Auditoría del modelo, primera vuelta**: cuatro barridos mecánicos sobre las 67 tablas | ✅ | [`auditoria-modelo-2026-08.md`](./auditoria-modelo-2026-08.md) · 4 hallazgos: dos errores de documentación publicada (corregidos), **5 tablas sin código** y una **regla de negocio no documentada**. Deja escrito el método y la trampa de medir contra una base vieja | 2026-08-24 |
| `TD7-r2` | D7 | **Decisión del dueño**: la cadena de contratación, ¿se implementa o se retira? | ⬜ | `aplications`, `offers` y las tres de origen de contrato **no las toca ningún código**, y el rol `GestorContratacion` promete tres cosas de las que solo existe una | — |
| `TD7-c5` | D7 | `dossiers.cedula` guarda lo mismo que `persons.cedula` | ✅ | **No era redundante: MENTÍA.** Se escribía una vez al crear el expediente y **nunca** se actualizaba (cero `UPDATE dossiers` en el backend), mientras la cédula de la ficha **sí** cambia. Medido antes: con la ficha en `0000000001`, el expediente devolvía `1122334455`. Nadie la buscaba —`findDossierByCedula`, pese al nombre, resuelve por `persons` y busca por `person_id`—, y su índice no servía a ninguna consulta. Hoy viaja en el mismo `SELECT`, desde la ficha, gracias a la FK de `TD7-c3`. **Probado de extremo a extremo por HTTP**: cambiada la cédula, el expediente devuelve la nueva. Contrato **idéntico**, ningún golden movido | 2026-08-24 |
